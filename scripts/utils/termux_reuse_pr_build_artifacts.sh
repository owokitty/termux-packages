...
ci_artifact_url() {
	curl --silent \
		-H "Authorization: token ${GITHUB_TOKEN}" \
		-H "Accept: application/vnd.github.v3+json" \
		"https://api.github.com/repos/owokitty/termux-packages/actions/runs/${1}/artifacts" \
...
[[ "${GITHUB_EVENT_NAME:-}" == "push" ]] && {
	...
	RELATED_PRS_QUERY="
	query {
		repository(owner: \"owokitty\", name: \"termux-packages\") {
		$(
	...
	echo "::group::Detected PR #${PRS[0]}: ${PR_COMMIT_TITLE} — https://github.com/owokitty/termux-packages/pull/${PRS[0]}"
	echo "${PR_COMMIT_BODY}"
	echo "::endgroup::"
	...
	if (( ${#PR_BASE_TO_HEAD_CHANGED_BUILDSYSTEM_FILES[*]} + ${#PR_BASE_TO_HEAD_CHANGED_DEPS[*]} + PR_CI_REUSE )); then
		...
		if [[ -n "${WORKFLOW_ID}" ]]; then
			echo "We can safely reuse CI artifacts from https://github.com/owokitty/termux-packages/actions/runs/${WORKFLOW_ID}"
